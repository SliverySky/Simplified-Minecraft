/*
    These codes are modified from "https://github.com/skywind3000/RenderHelp/blob/master/RenderHelp.h"
*/
#ifndef BITMAP_H
#define BITMAP_H
#include <fstream>
...
public:
    ...
    inline void DrawLine(int x1, int y1, int x2, int y2, uint32_t color) {
        ...
        if (x1 == x2 && y1 == y2){
            ...
            if (dx >= dy){
                ...
                SetPixel(x2, y2, color); // 这里为什么要额外再画x2, y2
            } else{
                ...
                SetPixel(x2, y2, color); // Q：这里为什么要额外再画x2, y2
            }
        }
    }
    ...
    inline static uint32_t vector_to_color(glm::ivec4 color){
        color = glm::clamp(color, 0, 255);
        return (color.r << 16) | (color.g << 8) | color.b | (color.a << 24);
    }

 	// 按照 Vec4f 画点
	inline void SetPixel(int x, int y, glm::ivec4 color) {
		SetPixel(x, y, vector_to_color(color));
	}   
